...
public class KeyExpansion_AES
{
	...
	{
        int newWord = 0;
        newWord ^= (int) STransform((byte) (word >>> 24)) & 0x000000ff;
        newWord <<= 8;

        newWord ^= (int) STransform((byte) ((word & 0xff0000) >>> 16)) &
                0x000000ff;
        newWord <<= 8;

        newWord ^= (int) STransform((byte) ((word & 0xff00) >>> 8)) &
                0x000000ff;
        newWord <<= 8;

        newWord ^= (int) STransform((byte) (word & 0xff)) & 0x000000ff;
	...
	{
        byte High = 0, Low = 0;
        High = (byte) ((byte) (value >> 4) & 0x0f);
        Low = (byte) (value & 0x0f);
        return sbox[High][Low];
    }
	
	public void keyExpansion(byte key[], int word[]) 
	{
		// Key Expansion implemented as asked in the question.
        int temp = 0;
        
        for(int i = 0; i < 4; i++) // Functionality requested in question, start with first 4.
        {
        	word[i] = toWord( key[4 * i], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]);
        }
        
        for(int i =4;i <44;i++) //from 4 to 44 in the array.
        {
        	temp = word[i - 1];
        	if(i % 4 == 0)
        	{
